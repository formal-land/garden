(** * NullifierK fixed-base window-point computational certificate

    The [vm_compute] half of the NullifierK x-coordinate agreement: for every
    window [w] (0..84) and digit [i] (0..7), the circuit fixed-base table's
    Lagrange-interpolated x-coordinate equals the x-coordinate of the computed
    Weierstrass multiple of the NullifierK generator [G].

    The window points come from the shared materialised table
    ([circuit_proof/nullifier_k/table.v]): [x_check_true] compares all 85*8
    entries against the circuit table by [Z.eqb], reading the multiples from
    the pasted literal [NullifierKFullTable.full_table_reduced], so its
    ...
    builder-form [NullifierKFullTable.full_table] and rewrites through
...
(** Keep the table constants opaque to unification/conversion in this file
    ...
    already a value). *)
Opaque NullifierKFullTable.full_table.
Opaque NullifierKFullTable.full_table_reduced.
...
Module NullifierKFixedWindowCert.
      ...
      (fun w : nat =>
         List.forallb
           (FixedBaseXCert.check_fn table default
              NullifierKFullTable.full_table_reduced w)
           (List.seq 0 8))
      (List.seq 0 85) = true.
  ...
  Lemma x_check_entry (w i : nat) (Hw : (w < 85)%nat) (Hi : (i < 8)%nat) :
    ...
    = Point.x (PallasModel.repr
         (List.nth i (List.nth w NullifierKFullTable.full_table [])
            Pallas.identity)).
  Proof.
    exact (FixedBaseXCert.x_check_entry table default
             NullifierKFullTable.full_table NullifierKFullTable.full_table_reduced 85
             NullifierKFullTable.full_table_reduced_eq x_check_true w i Hw Hi).
  Qed.
End NullifierKFixedWindowCert.
