(** * value_commit_r fixed-base window-point computational certificate

    The [vm_compute] half of the value_commit_r x-coordinate agreement: for
    ...
    computed Weierstrass multiple of the value_commit_r generator [G].

    The window points come from the shared materialised table
    ([circuit_proof/value_commit_r/table.v]): [x_check_true] compares all
    85*8 entries against the circuit table by [Z.eqb], reading the multiples
    from the pasted literal [ValueCommitRFullTable.full_table_reduced], so its
    ...
    builder-form [ValueCommitRFullTable.full_table] and rewrites through
...
(** Keep the table constants opaque to unification/conversion in this file
    ...
    already a value). *)
Opaque ValueCommitRFullTable.full_table.
Opaque ValueCommitRFullTable.full_table_reduced.

Module ValueCommitRFixedWindowCert.
  (** Alias for the circuit table / default window. *)
  Definition table : EccSpec.fixed_table :=
    OrchardCircuitSpec.value_commit_r OrchardActionFixedBase.orchard_internal_params.
      ...
      (fun w : nat =>
         List.forallb
           (FixedBaseXCert.check_fn table default
              ValueCommitRFullTable.full_table_reduced w)
           (List.seq 0 8))
      (List.seq 0 85) = true.
  ...
  Lemma x_check_entry (w i : nat) (Hw : (w < 85)%nat) (Hi : (i < 8)%nat) :
    ...
    = Point.x (PallasModel.repr
         (List.nth i (List.nth w ValueCommitRFullTable.full_table [])
            Pallas.identity)).
  Proof.
    exact (FixedBaseXCert.x_check_entry table default
             ValueCommitRFullTable.full_table ValueCommitRFullTable.full_table_reduced 85
             ValueCommitRFullTable.full_table_reduced_eq x_check_true w i Hw Hi).
  Qed.
End ValueCommitRFixedWindowCert.
