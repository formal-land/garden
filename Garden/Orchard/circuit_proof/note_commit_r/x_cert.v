(** * note_commit_r fixed-base window-point computational certificate

    The [vm_compute] half of the note_commit_r x-coordinate agreement: for
    ...
    computed Weierstrass multiple of the note_commit_r generator [G].

    The window points come from the shared materialised table
    ([circuit_proof/note_commit_r/table.v]): [x_check_true] compares all
    85*8 entries against the circuit table by [Z.eqb], reading the multiples
    from the pasted literal [NoteCommitRFullTable.full_table_reduced], so its
    ...
    builder-form [NoteCommitRFullTable.full_table] and rewrites through
...
(** Keep the table constants opaque to unification/conversion in this file
    ...
    already a value). *)
Opaque NoteCommitRFullTable.full_table.
Opaque NoteCommitRFullTable.full_table_reduced.

Module NoteCommitRFixedWindowCert.
      ...
      (fun w : nat =>
         List.forallb
           (FixedBaseXCert.check_fn table default
              NoteCommitRFullTable.full_table_reduced w)
           (List.seq 0 8))
      (List.seq 0 85) = true.
  ...
  Lemma x_check_entry (w i : nat) (Hw : (w < 85)%nat) (Hi : (i < 8)%nat) :
    ...
    = Point.x (PallasModel.repr
         (List.nth i (List.nth w NoteCommitRFullTable.full_table [])
            Pallas.identity)).
  Proof.
    exact (FixedBaseXCert.x_check_entry table default
             NoteCommitRFullTable.full_table NoteCommitRFullTable.full_table_reduced 85
             NoteCommitRFullTable.full_table_reduced_eq x_check_true w i Hw Hi).
  Qed.
End NoteCommitRFixedWindowCert.
