(** * The verified MSM checker for the pinned vk commitments
    ...
    certified SRS generators [g] ([Orchard/vk_srs_cert.v]).  Computing
    ...
    - [commit_two_shards] packages the chain for a leaf certificate: two
      Pippenger half-range checkpoints, the [intt] coefficient
      certificate, the [+ w] blind term, and the coordinate equality with
      the pinned literal.

    Scalars live modulo [pallas_p] ([VkPinnedData.scalar_modulus], the
    Vesta group order); point coordinates modulo [pallas_q].  This file
...
Require Import Garden.Orchard.vk_srs_cert.
...
(** The half-order fact the FFT butterflies consume:
    [omega_inv^1024 = -1 mod p].  The power is certified through the
    square-and-multiply [modpow] and transferred by rewriting only — a
    bare [Z.pow] reaching any conversion engine would build the unreduced
    260kbit integer. *)
Lemma omega_inv_half_modpow :
  modpow (p := Primes.pallas_p) omega_inv 1024 = scalar_p - 1.
Proof. vm_cast_no_check (@eq_refl Z (Primes.pallas_p - 1)). Qed.
...
Proof.
  pose proof omega_inv_half_modpow as H.
  rewrite (modpow_correct (p := Primes.pallas_p) omega_inv 1024) in H
    by lia.
  unfold Fpow, UnOp.from in H.
  exact H.
Qed.
...
(** ** The certified SRS as points *)

Definition aff (xy : Z * Z) : point := Weierstrass.Affine (fst xy) (snd xy).

Definition g_points : list point := List.map aff VkSrsData.g_points.
Definition w_point : point := aff VkSrsData.w_point.

Lemma g_points_length : List.length g_points = 2048%nat.
Proof.
  unfold g_points. rewrite List.length_map.
  exact VkSrsCert.vk_srs_g_points_length.
Qed.

Lemma srs_good : Forall good g_points /\ good w_point.
Proof.
  pose proof VkSrsCert.vk_srs_points_valid as Hall.
  apply Forall_app in Hall. destruct Hall as [Hg Hw].
  split.
  - unfold g_points.
    apply Forall_map.
    eapply Forall_impl; [| exact Hg].
    cbv beta. intros xy [Ho Hr]. split; assumption.
  - inversion Hw; subst.
    destruct H1 as [Ho Hr]. split; assumption.
Qed.

Lemma g_points_good : Forall good g_points.
Proof. exact (proj1 srs_good). Qed.

Lemma w_point_good : good w_point.
Proof. exact (proj2 srs_good). Qed.
...
(** The commitment of a full 2048-row Lagrange column, with the mandatory
    [Blind::default() = 1] term: [sum_j v_j g_lagrange_j + w]. *)
Definition commit_lagrange (v : list Z) : point :=
  padd (msm v g_lagrange_list) w_point.
...
Proof.
  revert j0. induction v as [| a v IH]; intros j0 Hv.
  ...
  - inversion Hv; subst.
    ...
    rewrite <- msm_zipadd;
      [ | rewrite !List.length_map; reflexivity | exact g_points_good].
    rewrite zipadd_map.
    f_equal.
Qed.
...
(** The linearity theorem (route (b)): the commitment over [g_lagrange]
    equals the [intt]-coefficient MSM over the raw certified [g]. *)
...
Proof.
  intros Hlen Hv.
  unfold commit_lagrange, g_lagrange_list.
  apply (f_equal (fun X : point => padd X w_point)).
  rewrite <- Hlen.
  rewrite (msm_glag_collect v 0 Hv).
  apply msm_ext_mod.
...
Proof.
  intros Hlen Hrange Hc Hcpa Hcpb Hfinal.
  ...
  { rewrite <- Hc.
    ...
    lia. }
  rewrite (commit_lagrange_intt v Hlen Hv).
  rewrite Hc.
  ...
  rewrite msm_app;
    [ | rewrite !List.firstn_length_le
        ...
        reflexivity
      | apply Forall_firstn; exact g_points_good
      | apply Forall_skipn; exact g_points_good].
  ...
  rewrite <- (pippenger_correct (List.skipn 1024 c)
    ...
    [ | rewrite !List.skipn_length, Hclen, g_points_length; reflexivity
      | apply Forall_skipn; exact g_points_good
