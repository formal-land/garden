...
Section Placed.
  ...
  Lemma placed_lookup_zero_of_determined
      (Γ : Assignment.t columns RegionId)
      (facts : list (Fact.t columns RegionId))
      (column : columns.(Columns.Lookup))
      (values : list Z) (default_value : Z)
      (Hconflict :
        Complete.no_conflicting_writes fixed_eqb lookup_eqb region_eqb facts
          = true)
      (Hdetermined : interpret_facts Γ (determined_facts facts))
      (Hin : List.In (Fact.LookupTableLoaded column values default_value) facts)
      (Hlen : (0 < List.length values)%nat) :
    Γ.(Assignment.lookup) column 0 =
    Complete.table_value lookup_eqb facts column 0.
  Proof.
    pose proof (interpret_facts_In Γ
      (Fact.LookupTableLoaded column values default_value)
      (determined_facts facts) Hdetermined
      (proj2 (List.filter_In fact_is_determined _ facts)
        (conj Hin eq_refl))) as Hfact.
    cbn [interpret_fact] in Hfact.
    rewrite (Hfact 0 (conj (Z.le_refl 0) (proj1 (Nat2Z.inj_lt 0 _) Hlen))).
    symmetry.
    exact (Complete.no_conflicting_writes_table fixed_eqb lookup_eqb region_eqb
      facts column values default_value Hconflict Hin 0).
  Qed.

  (** ** The placed hypotheses are weaker than the honest ones

      Sanity check on the statement: an assignment whose selector plane is
      the region-local enabled-point indicator ([Complete.honest_selector_plane],
      the hypothesis of [Complete.circuit_holds_intro]) satisfies
      [placed_selector_off] at every placement, and one whose lookup plane is
      the loaded table ([Complete.honest_lookup_plane]) satisfies the
      table-row-[0] equation.  So [placed_circuit_holds_intro] subsumes
      [Complete.circuit_holds_intro] modulo the (weaker) per-point
      obligations; the converse fails at a realized assignment, which is the
      reason this file exists. *)
  Lemma placed_selector_off_of_honest
      (Γ : Assignment.t columns RegionId)
      (idx : Indices.t columns) (rs : RegionId -> Z)
      (facts : list (Fact.t columns RegionId)) :
    Complete.honest_selector_plane selector_eqb region_eqb Γ facts ->
    placed_selector_off Γ idx rs facts.
  Proof.
    intros Hplane selector region row Hmemb.
    unfold Complete.honest_selector_plane in Hplane.
    rewrite Hplane.
    destruct (Complete.enabled_memb selector_eqb region_eqb facts selector
      region row) eqn:Henabled; [| reflexivity].
    exfalso.
    enough (placed_memb rs facts selector region row = true) as Htrue.
    { unfold placed_memb in Htrue. congruence. }
    exact (placed_memb_complete rs facts selector region row region row
      (Complete.enabled_memb_sound selector_eqb selector_eqb_spec region_eqb
        region_eqb_spec facts selector region row Henabled)
      eq_refl).
  Qed.

  Lemma placed_lookup_zero_of_honest
      (Γ : Assignment.t columns RegionId)
      (facts : list (Fact.t columns RegionId)) :
    Complete.honest_lookup_plane lookup_eqb Γ facts ->
    forall column,
      Γ.(Assignment.lookup) column 0 =
      Complete.table_value lookup_eqb facts column 0.
  Proof.
    intros Hplane column.
    exact (Hplane column 0 (Z.le_refl 0)).
  Qed.
