...
Definition interpolated_x
  ...
  List.fold_left
    (fun acc '(power, coeff) =>
      acc ➕
        (Garden.Halo2.halo2_gadgets.utilities.pow_expr window power
          ✖️ Expression.Fixed coeff Rotation.cur))
    (List.combine
      (List.seq 0 Garden.Halo2.halo2_gadgets.ecc.chip.constants.h_nat)
      lagrange_coeffs)
...
Definition coords_check
  ...
  let y_check :=
    Garden.Halo2.halo2_gadgets.utilities.square u ➖ y_p ➖ z in
  let on_curve :=
    Garden.Halo2.halo2_gadgets.utilities.square y_p
      ➖ Garden.Halo2.halo2_gadgets.utilities.square x_p ✖️ x_p
      ➖ Expression.Constant Garden.Halo2.halo2_gadgets.ecc.chip.constants.pallas_b in
  [
    (Some "check x", Constraint.Equal (interpolated_x window) x_p);
    (Some "check y",
      Constraint.Equal
        (Garden.Halo2.halo2_gadgets.utilities.square u ➖ y_p)
        z);
...
Definition running_sum_coordinates_check_gate : Gate.t columns := {|
  ...
  Gate.constraints :=
    ...
    let word :=
      z_cur ➖
        (z_next ● Garden.Halo2.halo2_gadgets.ecc.chip.constants.h) in
...
Definition configure : 𝓒 columns unit :=
  do🞵
    Garden.Halo2.halo2_gadgets.utilities.decompose_running_sum.configure
...
Definition synthesize
  ...
  do🞵 Garden.Halo2.halo2_gadgets.utilities.decompose_running_sum.synthesize in
  𝓛.AddRegion
