...
Definition Main : M.t (BlockUnit.t Empty_set) :=
  M.template_body [] (
    ...
    do~ M.while [[ InfixOp.lesser ~(| M.var (| "i" |), 16 |) ]] (
      do~ M.substitute_var "out" [Access.Array (M.var (| "i" |)); Access.Array (0)] [[ M.var_access (| "escalarMul", [Access.Array (M.var (| "i" |)); Access.Array (0)] |) ]] in
      do~ M.substitute_var "out" [Access.Array (M.var (| "i" |)); Access.Array (1)] [[ M.var_access (| "escalarMul", [Access.Array (M.var (| "i" |)); Access.Array (1)] |) ]] in
...
Definition Main_not_under_constrained : Prop :=
  exists! out,
  let signals := MainSignals.Build_t out in
