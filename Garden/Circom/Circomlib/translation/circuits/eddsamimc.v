...
(* Template signals *)
Module EdDSAMiMCVerifierSignals.
  Record t : Set := {
    (* Input *)
    enabled : F.t;
    (* Input *)
  ...
  Module IsNamed.
    ...
    | M : P _ M "M".
  End IsNamed.
End EdDSAMiMCVerifierSignals.

(* Template body *)
Definition EdDSAMiMCVerifier : M.t (BlockUnit.t Empty_set) :=
  M.template_body [] (
    (* Signal Input *)
    ...
    do~ M.substitute_var "hash" [] [[ M.call_function ~(| "MultiMiMC7", [ 5; 91 ] |) ]] in
    do~ M.substitute_var "hash" [Access.Component "in"; Access.Array (0)] [[ M.var (| "R8x" |) ]] in
    do~ M.substitute_var "hash" [Access.Component "in"; Access.Array (1)] [[ M.var (| "R8y" |) ]] in
    do~ M.substitute_var "hash" [Access.Component "in"; Access.Array (2)] [[ M.var (| "Ax" |) ]] in
    do~ M.substitute_var "hash" [Access.Component "in"; Access.Array (3)] [[ M.var (| "Ay" |) ]] in
    do~ M.substitute_var "hash" [Access.Component "in"; Access.Array (4)] [[ M.var (| "M" |) ]] in
...
Definition EdDSAMiMCVerifier_not_under_constrained enabled Ax Ay S R8x R8y M : Prop :=
  let signals := EdDSAMiMCVerifierSignals.Build_t enabled Ax Ay S R8x R8y M in
  True (* NotUnderConstrained EdDSAMiMCVerifier signals *).
