...
Definition EdDSAPoseidonVerifier : M.t (BlockUnit.t Empty_set) :=
  M.template_body [] (
    ...
    do~ M.substitute_var "hash" [Access.Component "inputs"; Access.Array (0)] [[ M.var (| "R8x" |) ]] in
    do~ M.substitute_var "hash" [Access.Component "inputs"; Access.Array (1)] [[ M.var (| "R8y" |) ]] in
    do~ M.substitute_var "hash" [Access.Component "inputs"; Access.Array (2)] [[ M.var (| "Ax" |) ]] in
    do~ M.substitute_var "hash" [Access.Component "inputs"; Access.Array (3)] [[ M.var (| "Ay" |) ]] in
    do~ M.substitute_var "hash" [Access.Component "inputs"; Access.Array (4)] [[ M.var (| "M" |) ]] in
    (* Component *)
    do~ M.declare_component "h2bits" in
    do~ M.substitute_var "h2bits" [] [[ M.call_function ~(| "Num2Bits_strict", ([] : list F.t) |) ]] in
    do~ M.substitute_var "h2bits" [Access.Component "in"] [[ M.var_access (| "hash", [Access.Component "out"] |) ]] in
    (* Component *)
...
Definition EdDSAPoseidonVerifier_not_under_constrained enabled Ax Ay S R8x R8y M : Prop :=
  let signals := EdDSAPoseidonVerifierSignals.Build_t enabled Ax Ay S R8x R8y M in
  True (* NotUnderConstrained EdDSAPoseidonVerifier signals *).
