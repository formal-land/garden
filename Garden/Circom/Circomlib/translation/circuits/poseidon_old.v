...
Definition Poseidon (nInputs : F.t) : M.t (BlockUnit.t Empty_set) :=
  ...
  do~ M.while [[ InfixOp.lesser ~(| M.var ~(| "i" |), InfixOp.add ~(| M.var ~(| "nRoundsF" |), M.var ~(| "nRoundsP" |) |) |) ]] (
    do~ M.substitute_var "ark" [[ M.call_function ~(| "Ark", [ M.var ~(| "t" |); M.var ~(| "C" |); InfixOp.mul ~(| M.var ~(| "t" |), M.var ~(| "i" |) |) ] |) ]] in
    (* Var *)
    do~ M.declare_var "j" [[ ([] : list F.t) ]] in
    do~ M.substitute_var "j" [[ 0 ]] in
    do~ M.while [[ InfixOp.lesser ~(| M.var ~(| "j" |), M.var ~(| "t" |) |) ]] (
      do~ M.if_ [[ InfixOp.eq ~(| M.var ~(| "i" |), 0 |) ]] (* then *) (
        do~ M.if_ [[ InfixOp.greater ~(| M.var ~(| "j" |), 0 |) ]] (* then *) (
          ...
          do~ M.substitute_var "ark" [[ 0 ]] in
          M.pure BlockUnit.Tt
        ) in
        M.pure BlockUnit.Tt
      ) (* else *) (
        do~ M.substitute_var "ark" [[ M.var_access ~(| "mix", [Access.Array (InfixOp.sub ~(| M.var ~(| "i" |), 1 |)); Access.Component "out"; Access.Array (M.var ~(| "j" |))] |) ]] in
        M.pure BlockUnit.Tt
      ) in
      do~ M.substitute_var "j" [[ InfixOp.add ~(| M.var ~(| "j" |), 1 |) ]] in
      M.pure BlockUnit.Tt
    ...
    do~ M.if_ [[ InfixOp.boolOr ~(| InfixOp.lesser ~(| M.var ~(| "i" |), InfixOp.div ~(| M.var ~(| "nRoundsF" |), 2 |) |), InfixOp.greaterEq ~(| M.var ~(| "i" |), InfixOp.add ~(| M.var ~(| "nRoundsP" |), InfixOp.div ~(| M.var ~(| "nRoundsF" |), 2 |) |) |) |) ]] (* then *) (
      ...
      do~ M.while [[ InfixOp.lesser ~(| M.var ~(| "j" |), M.var ~(| "t" |) |) ]] (
        ...
        do~ M.substitute_var "sigmaF" [[ M.var_access ~(| "ark", [Access.Array (M.var ~(| "i" |)); Access.Component "out"; Access.Array (M.var ~(| "j" |))] |) ]] in
        do~ M.substitute_var "mix" [[ M.var_access ~(| "sigmaF", [Access.Array (M.var ~(| "k" |)); Access.Array (M.var ~(| "j" |)); Access.Component "out"] |) ]] in
        do~ M.substitute_var "j" [[ InfixOp.add ~(| M.var ~(| "j" |), 1 |) ]] in
        M.pure BlockUnit.Tt
      ) in
      M.pure BlockUnit.Tt
      ...
      do~ M.substitute_var "sigmaP" [[ M.var_access ~(| "ark", [Access.Array (M.var ~(| "i" |)); Access.Component "out"; Access.Array (0)] |) ]] in
      do~ M.substitute_var "mix" [[ M.var_access ~(| "sigmaP", [Access.Array (M.var ~(| "k" |)); Access.Component "out"] |) ]] in
      (* Var *)
      do~ M.declare_var "j" [[ ([] : list F.t) ]] in
      do~ M.substitute_var "j" [[ 1 ]] in
      do~ M.while [[ InfixOp.lesser ~(| M.var ~(| "j" |), M.var ~(| "t" |) |) ]] (
        do~ M.substitute_var "mix" [[ M.var_access ~(| "ark", [Access.Array (M.var ~(| "i" |)); Access.Component "out"; Access.Array (M.var ~(| "j" |))] |) ]] in
        do~ M.substitute_var "j" [[ InfixOp.add ~(| M.var ~(| "j" |), 1 |) ]] in
        M.pure BlockUnit.Tt
      ) in
      M.pure BlockUnit.Tt
