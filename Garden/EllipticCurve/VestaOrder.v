(** * The Vesta group order annihilates every point
    ...
    a kernel-checked [pallas_p]-fold double-and-add ladder as its order
    certificate. *)
...
Module VestaOrder.
  ...
  Lemma vesta_q_prime : IsPrime Vesta.vesta_q.
  ...
  (** Prime-order certificate: a finite, kernel-checked
      [pallas_p]-fold double-and-add ladder. *)
  Lemma placeholder_order :
    Vesta.mul Vesta.vesta_q (Vesta.affine (-1) 2) = Vesta.identity.
  Proof. vm_compute. reflexivity. Qed.
  ...
  Lemma vesta_mul_mod (P : Vesta.point) (n : Z) :
    Vesta.reduced P ->
    Vesta.on_curve P ->
    Vesta.mul (n mod Vesta.vesta_q) P = Vesta.mul n P.
  Proof.
    intros HrP HoP.
    apply (GroupOrderTight.mul_mod_order Vesta.a Vesta.b eleven_lt_q
             Vesta.nonsingular P Vesta.vesta_q n).
    - pose proof (Znumtheory.prime_ge_2 _ vesta_q_prime). lia.
    - exact HrP.
    - exact HoP.
