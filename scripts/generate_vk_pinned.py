...
* ``Garden/Orchard/vk/data.v``: deployed right-hand-side targets for the
  fields derived by Garden's provenance certificates;
...
def render_data_v(
    ...
    return f"""(** * Deployed verifying-key targets for provenance and byte parity.

    The components of the pinned verifying-key description
    ([{source}]): the
    base/scalar modulus strings, the [extended_k] domain constant,
    [minimum_degree], and the 44 commitment coordinate pairs (29
    fixed-column + 15 permutation commitments, affine Vesta points).
    These literals are equality targets, not assumptions.  The setup and
    compilation certificates derive the non-commitment fields from the
    selected curve, [k = {domain['k']}], and Garden's formal configure program.  The
    commitment provenance certificate derives every coordinate from the
    model's column evaluations, [Params::new(11)], the inverse FFT, and the
    MSM in an optimized executable Rocq model.  The same target values are
    certified byte-for-byte against the deployed dump by T1
    ([vk/parity.v]).
...
(** Deployed equality target for the extended evaluation-domain exponent.
    [OrchardVkSetup] recomputes it from the selected [k = {domain['k']}], the Vesta
    scalar field's two-adicity, and the compiled constraint-system degree. *)
Definition extended_k : Z := {domain['extended_k']}.

(** Deployed equality target for the optional configured minimum degree.
    The configure metadata interpreter independently derives [{minimum_degree_rocq}]. *)
