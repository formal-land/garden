...
def render_data_v(
    ...
    the model computes no MSM, so the coordinates are witness input
    exactly like the fingerprint literals of [compiled/pinned.v]).
    Untrusted witness input, certified byte-for-byte against the dump
    by the T1 parity certificate ([vk/parity.v]).
